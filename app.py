...
def conectar_db():
    #Estalbelece conexão com o banco de dados
...
def encerrar_db(cursor, conexao):
    #fechar cursor e conexao cpm Banco de Dados
    cursor.close()
    conexao.close()
...
def cadastro_usuario():
    #Verificar se o acesso a essa rota é o ADM
    if 'adm' not in session:
        return redirect('/login')
    #Verifica se o acesso fo via formulário
...
def editar_usuario(id): 
    #Verificar se o acesso a essa rota é do ADM
    if 'adm' not in session:
        return redirect('/login')
...
def excluir_usuario(id):
    ...
    #Excluirá os posts do usuario a ser excluído
    conexaoDB = conectar_db()
    cursorDB = conexaoDB.cursor()
...
#ROTA TRATA ERO 4040 - PÁGINA NÃO ENCONTRADA
